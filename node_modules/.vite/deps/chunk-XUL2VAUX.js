...
// node_modules/@splinetool/runtime/build/runtime-chunk-TKCOO74F.js
function D(e3) {
...
function I(e3) {
  ...
  for (let i2 of e3.getLayersOfType("reflection")) {
    ...
    let r = "number" == typeof e4.blur ? e4.blur : null;
    t = Math.min(t, r ?? 0), a = Math.max(a, r ?? 10);
  }
...
function P2(e3) {
  for (let t of e3.getLayersOfType("reflection")) {
    ...
    let { blur: a, thickness: i2, steps: r, stepSize: s3 } = e4;
    return "number" != typeof a || "number" != typeof i2 || "number" != typeof r || "number" != typeof s3 ? "variable" : { blur: a, thickness: i2, steps: r, stepSize: s3, tracingMode: 1 === e4.tracingMode ? 1 : 0 };
  }
  return null;
}
function V3(e3, t) {
  let a = { outline: false, cavity: false }, i2 = 512 & e3.layers.mask, r = t.getLayersOfType("transmission"), s3 = t.getLayersOfType("outline"), n = x(t), h2 = t.getLayersOfType("cavity");
  return (s3.length > 0 || h2.length > 0) && (e3.layers.set(8), r.length > 0 && e3.layers.enable(3), n > 0 && e3.layers.enable(4), a.outline = s3.length > 0, a.cavity = h2.length > 0, s2(e3), i(e3)), 0 === r.length && 0 === s3.length && 0 === h2.length && 0 === n && e3.layers.set(0), i2 && e3.layers.set(9), !i2 && D(e3) && e3.layers.enable(5), z(e3), a;
}
function G(e3, t) {
  if (!t.layers) return false;
  let a = 512 & e3.layers.mask, i2 = false, r = t.getLayersOfType("transmission").filter((e4) => e4.data.visible), s3 = t.getLayersOfType("outline").filter((e4) => e4.data.visible), n = x(t), l = t.getLayersOfType("cavity").filter((e4) => e4.data.visible);
  return r.length > 0 && (e3.layers.set(3), (s3.length > 0 || l.length > 0) && e3.layers.enable(8), n > 0 && e3.layers.enable(4), i2 = true), 0 === r.length && 0 === s3.length && 0 === l.length && 0 === n && e3.layers.set(0), a && e3.layers.set(9), !a && D(e3) && e3.layers.enable(5), z(e3), i2;
}
function Z(e3, t) {
  var a;
  if (!t.layers) return false;
  let i2 = 512 & e3.layers.mask, r = false, s3 = x(t), n = t.getLayersOfType("transmission").filter((e4) => e4.data.visible), l = t.getLayersOfType("outline").filter((e4) => e4.data.visible), o = t.getLayersOfType("cavity").filter((e4) => e4.data.visible);
  return s3 > 0 && ((a = e3.userData).__splinePlanarMirror ?? (a.__splinePlanarMirror = 0), e3.layers.set(4), n.length > 0 && e3.layers.enable(3), (l.length > 0 || o.length > 0) && e3.layers.enable(8), r = true), 0 === s3 && 0 === n.length && 0 === l.length && 0 === o.length && e3.layers.set(0), i2 && e3.layers.set(9), !i2 && D(e3) && e3.layers.enable(5), z(e3), r;
}
function Q(e3) {
  ...
  return e3.traverseEntity((e4) => {
    if (e4 instanceof ar) if (Array.isArray(e4.material)) for (let t2 = 0; t2 < e4.material.length; t2++) a(V3(e4, e4.material[t2]));
    else a(V3(e4, e4.material));
  }), t;
}
function W(e3) {
  let t = false;
...
function _(e3) {
  let t = false;
  return e3.traverseEntity((e4) => {
    if (e4 instanceof ar) if (Array.isArray(e4.material)) for (let a = 0; a < e4.material.length; a++) Z(e4, e4.material[a]) && (t = true);
    else Z(e4, e4.material) && (t = true);
  }), t;
}
var K = new Dl();
var N = new As();
var R = new Ra();
var Y = new Kr();
function J(e3, t, a, i2 = false) {
  let r = e3.cloner;
  if (r) {
    let s3 = null;
    if (e3.raycast === Za.prototype.raycast || e3.raycast === ie.prototype.raycast) {
      let t2 = e3.geometry;
      t2 && (null === t2.boundingSphere && t2.computeBoundingSphere(), s3 = t2.boundingSphere);
    }
    for (let n of r.children) {
      if (null !== s3 && (Y.copy(s3).applyMatrix4(n.matrixWorld), !t.ray.intersectsSphere(Y))) continue;
      let r2 = N.copy(n.matrixWorld).invert(), l = R.copy(t.ray).applyMatrix4(r2), o = e3.matrixWorld;
      l.applyMatrix4(o);
      let h2 = K;
...
var X = class extends Dt(hr) {
  ...
  updateByOp(e3, t, a, i2) {
    let r = this.data.uiFrame;
    super.updateByOp(e3, t, a, i2);
    let s3 = t;
    V(e3.path, ["fog"]) ? this.updateFog(s3.fog, a.shared) : V(e3.path, ["sky"]) ? this.updateSky(s3.sky, a.shared) : V(e3.path, ["grading"]) ? this.gradingState = s3.grading ?? null : V(e3.path, ["ao"]) ? this.updateAo(s3.ao, a.shared) : V(e3.path, ["ambient"]) ? this.updateAmbientLight(s3.ambient, a.shared) : V(e3.path, ["shadow"]) ? this.updateShadow(s3.shadow) : 0 === e3.type && 0 === e3.path.length && (void 0 !== e3.props.uiFrame || r !== s3.uiFrame) && this.updateUIFrame(e3.props.uiFrame, a);
  }
  updateUIFrame(e3, t) {
  ...
  updatePenumbraSizeArray() {
    ...
    this.traverseEntity((r) => {
      ...
      r instanceof p && r.visible && a < ui && (this.penumbraSizeArrayCache[a] = r.data.penumbraSize ?? 0.5, a += 1), r instanceof v && r.visible && a < di - ui && (this.penumbraSizeArrayCache[ui + i2] = r.data.penumbraSize ?? 0.5, i2 += 1);
    });
  }
  raycastWithClones(e3) {
    let t = [], a = (s3) => {
      for (let n of s3.children) {
        let s4 = n.cloner;
        mt.is(n) && (n.visible || (s4 == null ? void 0 : s4.object.data.visible)) && ((P(n) || k(n) && this.scene.enableHelpers && n.objectHelper.visible) && (e3.intersectObject(n, false, t), J(n, e3, t, true)), a(n));
      }
    };
    return a(this), t;
  }
...
//# sourceMappingURL=chunk-XUL2VAUX.js.map
