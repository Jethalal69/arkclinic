...
// node_modules/@splinetool/runtime/build/runtime-components-QFBU6NQX.js
var I = class extends l(Ks, c2) {
  constructor(e2, t, n) {
  ...
  expandInstanceChildren(t) {
    ...
    if (void 0 === this.component) {
      ...
      if (s) {
        let e2 = {};
        D(t, [this.uuid], n.overrides, this, s, s, 0, r, e2);
...
function D(e2, t, n, i2, r, a2, p, d, l2) {
  ...
  r instanceof I && r.isInstanceRoot && r.expandInstanceChildren(e2);
  let c3 = 0;
  for (let f2 of r.children) if (mt.is(f2)) {
    ...
    if (!d) {
      ...
      if (j = mt.is(t2) ? t2 : null, null !== j) {
        let e3 = k(j, f2, v, r2);
        g = e3 >= 1 ? j.stateSelection : void 0, 2 !== e3 && (j = null);
      }
      if (null === j && (j = e2.scene.findInstance(r2) ?? null, null !== j)) {
        let t3 = k(j, f2, v, r2);
        if (g = t3 >= 1 ? j.stateSelection : void 0, 2 !== t3) j = null;
        else {
          let t4 = j.parent.children.indexOf(j);
          j.parent.children.splice(t4, 1), i2.children.splice(c3, 0, j), j.parent === i2 ? t4 <= c3 && console.error("not possible") : (j.parent = i2, j.matrixWorldNeedsUpdate = true, j.resetBBoxNeedsUpdate(), j.updateVisible(), e2.pendingDeletes.delete(j));
        }
      }
    }
    if (null === j) {
      let t2 = v ? M.apply(f2.data, v) : f2.data;
      nt.is(t2.type) && (t2 = { ...t2, type: "Empty" }), j = C.createEntity(r2, t2, e2), j.overrideData = v, i2.add(j), i2.children.splice(i2.children.length - 1, 1), i2.children.splice(c3, 0, j), j.updateState(j.data, e2), g && j.changeSelectedState(g, e2), e2.scene.registerInstanceAndSetUuid(j);
    }
    j.isBone && (l2[j.identity[j.identity.length - 1]] = j.uuid), c3 += 1, D(e2, t, n, j, f2, a2, p + 1, d, l2);
  }
  if (!d) {
    let t2 = c3;
    for (; ; ) {
...
var b = (e2, t, n) => new I(e2, t, n);
c("Component", b), c("Instance", b), a("components");
//# sourceMappingURL=runtime-components-QFBU6NQX-4JTV4PJK.js.map
