...
// node_modules/@splinetool/runtime/build/runtime-paths-4JBIN2NB.js
...
var N = class extends ha {
  ...
  build() {
    ...
    if ("round" === v2) {
      let e2 = this.inputs.shapeData, t2 = F.get(e2);
      void 0 !== t2 && t2.bevel === y && t2.curveSegments === w && t2.bevelSides === f ? S = t2.geometry : (S = new Lt(e2, 2 * y, y, w, f, void 0, true), F.set(e2, { bevel: y, curveSegments: w, bevelSides: f, geometry: S })), O2 = S.getAttribute("position").count;
    }
    let R2 = 0, T2 = 0;
    M.sort((e2, t2) => e2.start - t2.start), M.forEach((e2) => {
      e2.verticesStart = R2, e2.verticesCount = e2.continuous.reduce((e3, t2, s2) => e3 + (0 !== s2 && t2 ? 1 : 2), 0), T2 += e2.verticesCount, R2 = T2;
    });
    let k2, E2 = T2 * o, z2 = 0;
    this._isOpenEnded() && "flat" === v2 && (k2 = this._getSurfaceTesselation(_), z2 = k2.vertexCount);
    let j2 = E2 + 2 * z2 + 2 * O2, G2 = E2 + 2 * z2, B2 = { positions: new Float32Array(3 * j2), normals: new Float32Array(3 * j2), uvs: new Float32Array(2 * j2) }, U2 = S ? S.getIndex().array : null, N2 = 0;
    M.forEach((e2) => {
      N2 += e2.count;
    });
    let H = N2 * (o - 1) * 6 + (k2 ? 2 * k2.elementCount * 3 : 0) + (U2 ? 2 * U2.length : 0), Z = new Uint32Array(H), q = 0;
    if (M.forEach((e2) => {
      q = this._extrudeRegion(e2, A2, u, l, B2, Z, q, this._isGeometryClosed() && !this._isOpenEnded());
    }), k2 && (q = this._closeEnd(k2, E2, Z, q, B2, u[0], l[0], false), q = this._closeEnd(k2, E2 + z2, Z, q, B2, u[u.length - 1], l[l.length - 1], true)), S) {
      B2.positions.set(S.getAttribute("position").array, 3 * G2), B2.normals.set(S.getAttribute("normal").array, 3 * G2), B2.uvs.set(S.getAttribute("uv").array, 2 * G2);
      for (let e3 = G2; e3 < G2 + O2; e3++) B2.uvs[2 * e3 + 1] = 1e-4;
      let e2 = q;
      for (let e3 = 0; e3 < U2.length; e3++) Z[q++] = U2[e3] + G2;
      G2 += O2, B2.positions.set(S.getAttribute("position").array, 3 * G2), B2.normals.set(S.getAttribute("normal").array, 3 * G2), B2.uvs.set(S.getAttribute("uv").array, 2 * G2);
      let t2 = q;
      for (let e3 = 0; e3 < U2.length; e3++) Z[q++] = U2[e3] + G2;
      this.setAttribute("position", new qr(B2.positions, 3)), this.setAttribute("normal", new qr(B2.normals, 3)), this.setAttribute("uv", new qr(B2.uvs, 2)), this._setIndexFromUint32(Z);
      let s2 = D;
      s2.copy(u[u.length - 1]).setPosition(l[l.length - 1]), this.applyMatrix4OnRange(s2, G2, j2), s2.copy(u[0]).setPosition(l[0]).multiply(I.makeScale(1, 1, -1)), this.applyMatrix4OnRange(s2, G2 - O2, G2), this.reverseIndicesOnRange(e2, t2);
    } else this.setAttribute("position", new qr(B2.positions, 3)), this.setAttribute("normal", new qr(B2.normals, 3)), this.setAttribute("uv", new qr(B2.uvs, 2)), this._setIndexFromUint32(Z);
  }
  ...
  _getSampledPath() {
    let e = this.inputs.path, t = ut(e), s = B.get(t);
    if (void 0 === s || s.subdivisions !== e.subdivisions || s.subdivisionMode !== e.subdivisionMode) {
      ...
      s = { subdivisions: e.subdivisions, subdivisionMode: e.subdivisionMode, points: i2, matrices: n }, B.set(t, s);
    }
    return s;
  }
  ...
  _computeBasisMatrices(e) {
    ...
    for (let h2 = 0; h2 < s; h2++) {
      let u2, p2, c2 = e[h2];
      u2 = 0 === h2 ? i2 ? e[s - 1] : R.copy(c2).multiplyScalar(2).sub(e[1]) : e[h2 - 1], p2 = h2 === s - 1 ? i2 ? e[0] : R.copy(c2).multiplyScalar(2).sub(e[h2 - 1]) : e[h2 + 1], T.subVectors(c2, u2).normalize(), k.subVectors(p2, c2).normalize();
      let d2 = E.addVectors(T, k);
      d2.lengthSq() < 1e-10 && (d2.copy(k), d2.dot(r) < 0 && d2.negate()), d2.normalize(), r.copy(d2), 0 === h2 && (d2.equals(l) || z.copy(d2).negate().equals(l)) && l.set(0, 0, 1);
      let m = j.crossVectors(l, d2).normalize(), g = G.crossVectors(d2, m).normalize();
  ...
  _getShapePoints(e, t = we.ODD) {
    let s = this.inputs.shapeData, n = U.get(s);
    return (void 0 === n || n.curveSegments !== e || n.windingRule !== t) && (n = { curveSegments: e, windingRule: t, shapePoints: this._computeShapePoints(e, t) }, U.set(s, n)), n.shapePoints;
  }
  _getSurfaceTesselation(e) {
    let t, s = U.get(this.inputs.shapeData);
  ...
  _computeShapePoints(e = 12, t = we.ODD) {
    ...
    for (let e2 = s.elementCount - 1; e2 >= 0; e2--) {
      ...
      do {
        ...
        let P = s.vertexIndices[l2];
        if (Array.isArray(P)) o.continuous[e3] = false;
        else {
          let [t3, s2] = h.getCurveIndexFromVertexId(P - 1, true);
  ...
  _extrudeRegion(e, t, s, i2, n, a2, r, o) {
    ...
    for (let t2 = 0; t2 < h; t2++) {
      let n2 = s[t2].elements, a3 = n2[0], r2 = n2[1], o2 = n2[2], m2 = n2[4], g2 = n2[5], y2 = n2[6], f2 = n2[12], v2 = n2[13], w = n2[14], x = i2[t2], S = t2 / (h - 1), b = 0, _ = 0, P = 0, C = e.verticesStart * h + e.verticesCount * t2;
      for (let e2 = 0; e2 < u; e2 += 6) {
        if (0 === l[e2 + 5]) {
          let t4 = l[e2 + 0], s3 = l[e2 + 1];
          b = a3 * t4 + m2 * s3 + f2 + x.x, _ = r2 * t4 + g2 * s3 + v2 + x.y, P = o2 * t4 + y2 * s3 + w + x.z;
        }
        let t3 = l[e2 + 2], s2 = l[e2 + 3], i3 = a3 * t3 + m2 * s2 + f2, n3 = r2 * t3 + g2 * s2 + v2, h2 = o2 * t3 + y2 * s2 + w, u2 = 1 / (Math.sqrt(i3 * i3 + n3 * n3 + h2 * h2) || 1);
        i3 *= u2, n3 *= u2, h2 *= u2;
        let M = 2 * C, A2 = 3 * C;
        p[A2 + 0] = b, p[A2 + 1] = _, p[A2 + 2] = P, c[A2 + 0] = i3, c[A2 + 1] = n3, c[A2 + 2] = h2, d[M + 0] = l[e2 + 4], d[M + 1] = S, C++;
      }
    }
...
//# sourceMappingURL=runtime-paths-4JBIN2NB-D67P7ZDO.js.map
