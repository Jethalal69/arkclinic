import {
  ...
  We,
  qe
} from "./chunk-STUVUO5R.js";
import "./chunk-TBSNN2RH.js";
...
// node_modules/@splinetool/runtime/build/runtime-transitions-YUNUNEOJ.js
function j(t2) {
  var _a;
...
var f = { state: null, duration: 0, delay: 0, delayDirection: "start-once", direction: "normal", repeat: 0, easing: 0 };
var y = class extends Fi {
  constructor(t2, i, s, r2, n3, a2) {
    var _a;
    if (super(), this.eventType = t2, this.data = i, this.page = s, this.shared = r2, this.condition = n3, this.runModeOverride = a2, this.tweens = [], this.hasPlayedOnce = false, this.inert = Te, !i.object) throw new Error("Missing property");
    let h2 = this.page.scene.find(i.object) ?? ((_a = this.page.uiCanvas) == null ? void 0 : _a.find(i.object)) ?? this.page.scene.find2DInUIObjects(i.object);
    if (!h2) throw new Error("Missing property");
    if (!h2.states) throw new Error("Missing property");
    if (this.object = h2, i.tweens.length < 2) throw new Error("Missing property");
    this.init();
  }
  get playing() {
    var _a;
  ...
  init() {
    this.initialCurrentState = Ie(this.object, this.object.currentState).id, this.build();
  }
  get runMode() {
  ...
  build() {
    ...
    try {
      ...
      void 0 !== t2 && e.push(new We(this.object, t2, t2, f, this.shared, this.data.allowSlerp ?? false, this.page, {}, () => this.inert));
      for (let t3 = 1; t3 < i.length; t3++) {
        let s = i[t3 - 1], n3 = i[t3];
        e.push(new We(this.object, void 0 === s.data.state ? this.currentState : s.data.state, void 0 === n3.data.state ? this.currentState : n3.data.state, n3.data, this.shared, this.data.allowSlerp ?? false, this.page, {}, () => this.inert));
      }
    } catch (t2) {
      return void (t2 instanceof Error && console.error(t2.message));
    }
    this.tweens = e, this.syncInert(), this.runner = new g({ ...Pe(j(this.data)), runMode: this.runMode }, e.map((t2) => t2.runner), { claim: () => this.claimObject(), assignCurrentState: () => this.rebindTweens(), isOwner: () => this.object.previousAction === this });
  }
  claimObject() {
    var _a;
  ...
  rebindTweens() {
    ...
    this.syncInert(), this.object instanceof V && this.object.wasMovedByUser && (this.object.wasMovedByUser = false);
  }
  syncInert() {
    this.inert = qe(this.tweens.flatMap((t2) => t2.endpointData));
...
var T = class extends Fi {
  constructor(t2, i, e, s, r2) {
    var _a, _b, _c;
    super(), this.data = t2, this.page = i, this.animationControls = e, this.shared = s, this.condition = r2, this.startOnceDelay = 0, this.toggleIsForward = false;
    let a2 = t2.object, h2 = i.scene.find(t2.object);
    h2 && Array.isArray(h2.identity) && (a2 = h2.identity[0]), this.data = { ...t2, object: a2 };
  ...
  play(t2 = false) {
    ...
    window.clearTimeout(this.timeoutId), this.timeoutId = window.setTimeout(() => {
      ...
      Object.entries(this.animationControls.clipIdToAction).forEach(([e, s]) => {
        let r2 = e.split("/")[1];
        this.threeAnimAction !== s && r2 === i && (t2 ? s.fadeOut(0.2) : s.stop());
      }), t2 && ((_a = this.threeAnimAction) == null ? void 0 : _a.fadeIn(0.2));
    }, this.startOnceDelay);
  }
  pause() {
    this.threeAnimAction && (this.threeAnimAction.paused = true), window.clearTimeout(this.timeoutId), delete this.timeoutId;
  }
  stop() {
    var _a;
  ...
  toggle(t2, i = false) {
    ...
    this.toggleIsForward = t2 ?? !this.toggleIsForward, this.threeAnimAction && (this.threeAnimAction._loopCount = 0), this.playing ? this.threeAnimAction && (this.threeAnimAction.timeScale = this.toggleIsForward ? 1 : -1, this.toggleIsForward ? (this.threeAnimAction._clip.start = this.leftFrameTime, this.threeAnimAction._clip.duration = this.rightFrameTime) : (this.threeAnimAction._clip.start = this.rightFrameTime, this.threeAnimAction._clip.duration = this.leftFrameTime)) : this.timeoutId = window.setTimeout(() => {
      ...
      if (this.threeAnimAction = this.animationControls.play(this.data), this.threeAnimAction) {
        ...
        Object.entries(this.animationControls.clipIdToAction).forEach(([e, s]) => {
          let r2 = e.split("/")[1];
          this.threeAnimAction !== s && r2 === t3 && s.isRunning() && (i ? s.fadeOut(0.2) : s.stop());
...
n(We), n2("Transition", ({ eventType: t2, data: i, page: e, shared: s, actionCondition: r2, runModeOverride: n3 }) => new y(t2, i, e, s, r2, n3)), n2("Animation", ({ data: t2, page: i, eventManager: e, shared: s, actionCondition: r2 }) => new T(t2, i, e.animationControls, s, r2)), a("transitions");
//# sourceMappingURL=runtime-transitions-YUNUNEOJ-X7N7QUCF.js.map
